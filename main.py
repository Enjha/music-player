...
def main():
    ...
    import_button = Button(window, text="Importer des musiques", bg="grey", fg="black",
                           command=lambda: Library.import_music(music_path, music_space))
    ...
    prev_button = Button(window, text="prev", image=prev_icon, bg='grey', borderwidth=0,
                         command=lambda: Music.prev_music(music_path, music_space, space))
    prev_button.pack(padx=8, pady=10, in_=buttons, side=LEFT)

    stop_icon = PhotoImage(file="ressources\\icons\\stop.png")
    stop_button = Button(window, text="stop", image=stop_icon, bg='grey', borderwidth=0,
                         command=lambda: Music.stop_music(music_space))
    stop_button.pack(padx=8, pady=15, in_=buttons, side=LEFT)

    play_icon = PhotoImage(file="ressources\\icons\\play.png")
    play_button = Button(window, text="play", image=play_icon, bg='grey', borderwidth=0,
                         command=lambda: Music.play_music(music_path, music_space, space))
    play_button.pack(padx=8, pady=15, in_=buttons, side=LEFT)

    pause_icon = PhotoImage(file="ressources\\icons\\pause.png")
    pause_button = Button(window, text="pause", image=pause_icon, bg='grey', borderwidth=0,
                          command=lambda: Music.pause_music(pause_button))
    pause_button.pack(padx=8, pady=15, in_=buttons, side=LEFT)

    next_icon = PhotoImage(file="ressources\\icons\\next.png")
    next_button = Button(window, text="next", image=next_icon, bg='grey', borderwidth=0,
                         command=lambda: Music.next_music(music_path, music_space, space))
