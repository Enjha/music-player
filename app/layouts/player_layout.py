...
class Player: 
    def main_window():
        ...
        supp_button = Button(supp_zone, text="🗑️ Supprimer", bg='red',borderwidth=0, command=lambda : library.delete_music(music_space, music_title, pause_button))
        ...
        music_space = Listbox(musics_frame, fg="white",width=70,height=20, bg="#2f3542",font=('helvetica',18), selectbackground="#4b4b4b", relief=FLAT, selectforeground="white", highlightthickness=0, activestyle=NONE, yscrollcommand=scrollbar.set)
        ...
        music_title = Label(label_zone, text="Veuillez choisir une musique en double-cliquant.",bg='#1e272e', fg='white', font=('poppin',22))
        ...
        play_button = Button(window, text="▶️", bg='#1e272e', fg='white',borderwidth=0, command=lambda : library.play_music(music_space, music_title, pause_button))
        play_button.pack(padx=8, pady=15, in_=buttons, side=LEFT)
        play_button['font'] = f 
        ...
        def check_event():
            for event in pygame.event.get():
                if event.type == MUSIC_END:
                    library.next_music(music_space, music_title, pause_button)
